...
class MascletImperiApp:
    def __init__(self, root, encryption_key=None):
        ...
        # Ahora inicializar message_system
        ...
        self.style.configure('TButton', background=self.style.lookup('TButton', 'background'))
        ...
        self.users_file = PathManager.get_file_path('users.json')
    ...
    def verificar_login(self):
        ...
        password = hashlib.sha256(self.password_var.get().encode()).hexdigest()
        
        if username in self.usuarios and self.usuarios[username]["password"] == password:
            print(f"Login exitoso - Usuario: {username}, Rol: {self.usuarios[username]['role']}")  # Debug
            self.username = username
            self.user_role = self.usuarios[username]["role"]
            self.is_logged_in = True
            
            if self.recordar_var.get():
                self.guardar_credenciales(username, self.password_var.get())
    ...
    def verificar_login(self):
        ...
        if username in self.usuarios and self.usuarios[username]["password"] == password:
            print("Intentando guardar credenciales...")  # Debug temporal
            self.username = username
            self.user_role = self.usuarios[username]["role"]
            self.is_logged_in = True
                
            if self.recordar_var.get():
                print("Checkbox marcado, guardando...")  # Debug temporal
                # Aquí está el cambio importante - guardamos la contraseña sin encriptar
                self.guardar_credenciales(username, self.password_var.get())
    ...
    #def verificar_login(self):
        ...
        #password = hashlib.sha256(self.password_var.get().encode()).hexdigest()
         ...
         #   self.is_logged_in = True
          ...
          #  self.crear_interfaz_principal()
        #else:
         #   self.password_var.set("")  # Limpiar solo la contraseña
          #  self.mostrar_error_login()
    ...
    #def actualizar_ficha(self):
        #"""Abre la ventana de actualización de ficha"""
        #try:
            #ventana_actualizacion = ActualizarFicha(self.root)
            #ventana_actualizacion.transient(self.root)
            #ventana_actualizacion.grab_set()
        #except Exception as e:
            #messagebox.showerror("Error", f"Error al abrir ventana de actualización: {e}")
    ...
    def crear_botones_principales(self, parent):
        ...
        #user_text = f"Usuario: {self.username if self.username else 'None'} | Rol: {self.user_role if self.user_role else 'None'}"
        #tk.Label(info_frame, text=user_text, bg='white', font=('Arial', 11)).pack(anchor='w')
        
        #self.datetime_label = tk.Label(info_frame, text="", bg='white', font=('Arial', 11))
